...
class FLIRDataset(Dataset):
    ...
    def __init__(self, root, img_size=[256,256], transform = None , rgb=True,train=True,grey=True):
        super().__init__()
        root = "F:\\data/FLIR_thermal_data_kaggle/FLIR_ADAS_1_3/"
        train = True
        self.grey= grey
        self.train = train
        ...
        self.rgb_list = glob(os.path.join(self.rgb_root, '*.jpg'))
    ...
    def make_label(self, rgb_path):
        ...
        ir_name = os.path.basename(rgb_name)[:-4] + '.jpeg'
        ...
        if self.train:
            return self.transform(ir), self.transform(rgb)
