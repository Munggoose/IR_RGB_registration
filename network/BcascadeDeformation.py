...
from .netR.netR import NetR
from .netT.utils import GANLoss
import itertools


class BranchCascadeRegistrator(nn.Module):
    ...
    def forward(self,ir,rgb):
        ...
        warped_ir, warped_rgb, ir_grid, rgb_grid,reg_term = self.netR(self.ir, self.rgb)
    ...
    def backward_T_and_R(self):
        ...
        self.loss_L1_RGB = self.cfg.lambda_recon * self.criterionL1(self.warped_rgb, self.fake_rgb) ## rgb 
        ...
        loss = self.loss_L1_TR + self.loss_L1_RT + 0.2 * self.loss_L1_RGB  + self.loss_GAN_TR + self.loss_GAN_RT + self.loss_smoothness
        loss.backward()

        return (self.loss_GAN_TR + self.loss_GAN_RT) , (self.loss_L1_RT + self.loss_L1_TR), self.loss_L1_RGB
