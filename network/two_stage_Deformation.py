...
class TwoStageDeformationGenerator(nn.Module):
    def __init__(self,cfg):
        super().__init__()
        self.device= 'cuda'
        self.cfg = cfg
        self.netT = MT.UnetGenerator(1,1,8,64)
        self.netAffine = AffineSTN(cfg)
        self.netDFG = UnetSTN(cfg)
    ...
    def setup_optimizers(self):
        ...
        self.optimizer_DFG = torch.optim.Adam(itertools.chain(self.netDFG.parameters()),lr=self.cfg.lr, betas=(0.5, 0.999))
        self.optimizer_Affine = torch.optim.Adam(itertools.chain(self.netAffine.parameters()),lr=self.cfg.lr, betas=(0.5, 0.999))
        
        self.optimizer_T = torch.optim.Adam(self.netT.parameters(), lr=self.cfg.lr, betas=(0.5, 0.999))
        self.optimizer_D = torch.optim.Adam(self.netD.parameters(), lr=self.cfg.lr, betas=(0.5, 0.999))
        
        self.optimizers.append(self.optimizer_T)
        self.optimizers.append(self.optimizer_D)
        self.optimizers.append(self.optimizer_DFG)
        self.optimizers.append(self.optimizer_Affine)
    ...
    def forward(self,ir,rgb):
        ...
        #Affine Stage - Stage1
        warped_ir_affine, homography, reg_term_affine = self.netAffine(self.ir, self.rgb)
        self.warped_ir_affine = warped_ir_affine
        self.affine_reg_term = reg_term_affine
        self.fake_rgb_affine = self.netAffine.warping(self.fake_rgb , homography)

        self.homography = homography
        #Deformation Stage - Stage2
        warped_ir, ir_grid,reg_term_dfg = self.netDFG(warped_ir_affine, self.rgb)
    ...
    def optimize_parameters(self):
        ...
        # Backward D
        self.set_requires_grad([self.netT, self.netDFG,self.netAffine], False)
        self.optimizer_D.zero_grad()  # set D_A and D_B's gradients to zero
        D_loss = self.backward_D()  # calculate gradients for D_A
        self.optimizer_D.step()  # update D_A and D_B's weights
        self.set_requires_grad([self.netT, self.netDFG,self.netAffine], True)

        # Backward translation and registration networks
        self.set_requires_grad([self.netD], False)
        self.optimizer_Affine.zero_grad()
        self.optimizer_DFG.zero_grad()
        ...
        self.optimizer_Affine.step()
        self.optimizer_DFG.step()
